...
export function configWriteTargetMissingMessage(scope = "Config write"): string {
  return `${scope} needs app-server’s versioned user config target (filePath and expectedVersion). Refresh Settings so Forge can read the latest config layers, then try again.`;
}
