 ...
 *       scale every UI text size. Forge's CSS is ~588 hardcoded-px font sizes
 ...
 *
 * Forge stores both under the desktop.hicodex.* namespace (browser
...
// CODEX-REF: base.css :65 `--codex-chat-code-font-size: 12px;` is Forge's
...
// conservative (10-20) because Forge spacing/padding stay fixed px while only
...
export function loadUiAppearance(storage: BrowserStorageLike | null): UiAppearancePreferences {
  ...
  try {
    const raw = readMigratedStorageValue(storage, FORGE_DESKTOP_CONFIG_KEYS.appearanceUiFontSize);
  ...
  try {
    const raw = readMigratedStorageValue(storage, FORGE_DESKTOP_CONFIG_KEYS.appearanceCodeFontSize);
    const parsed = raw === null ? Number.NaN : Number(raw);
  ...
  try {
    const raw = readMigratedStorageValue(storage, FORGE_DESKTOP_CONFIG_KEYS.appearanceReducedMotion);
...
export function saveUiFontSize(storage: BrowserStorageLike | null, size: number): void {
  if (!storage) return;
  try {
    setDesktopAppSettingValue(storage, FORGE_DESKTOP_CONFIG_KEYS.appearanceUiFontSize, String(clampUiFontSize(size)));
  } catch {
...
export function saveUiCodeFontSize(storage: BrowserStorageLike | null, size: number): void {
  if (!storage) return;
  try {
    setDesktopAppSettingValue(storage, FORGE_DESKTOP_CONFIG_KEYS.appearanceCodeFontSize, String(clampCodeFontSize(size)));
  } catch {
    // Preference still applies for this session when storage is unavailable.
  }
}
...
export function saveUiReducedMotion(storage: BrowserStorageLike | null, mode: ReducedMotionMode): void {
  if (!storage) return;
  try {
    setDesktopAppSettingValue(storage, FORGE_DESKTOP_CONFIG_KEYS.appearanceReducedMotion, mode);
  } catch {
    // Preference still applies for this session when storage is unavailable.
  }
}
