...
export const YUXI_CATEGORIES: readonly YuxiCategoryMeta[] = [
  ...
  { key: "wording", label: "话术与包装库", line: "training_presales", kind: "proposal", description: "客户异议回应话术、对客口径、内容包装样例" },
...
export const YUXI_LIBRARY_GOVERNANCE: Readonly<Record<string, YuxiLibraryGovernance>> = {
  ...
  wording: {
    ownerRole: "售前方案组 / 市场",
    updateRule: "新话术验证有效或口径变化后更新",
    authorityRule: "已审批口径优先，草稿口径需标记待确认",
    citationScope: "对客表达、异议回应、内容润色",
    qualityMetrics: ["口径状态", "适用场景", "禁用表述"],
    externalSystems: ["知识运营后台"],
    uploadChecklist: ["话术主题", "适用场景", "禁用边界", "来源负责人"],
    matchSignals: ["客户问题", "行业语境", "风险词", "审批状态"],
  },
  bid_info: {
    ownerRole: "投标经理",
