...
- `ensure_default_forge_profile` creates or refreshes `config.toml`.
...
That mirrors the path already used by Codex CLI on this machine. If the directory is absent, Forge leaves the isolated config unchanged and the settings page continues to show a safe fallback.
