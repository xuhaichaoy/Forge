...
本文整理基于本 demo（`examples/internal-api-demo/`）讨论得出的、把 Forge
...
| **MCP Server** | 一个独立进程，把业务能力包装成工具 | `mcp-server/src/server.ts` | Node 子进程，Forge 启动时拉起 |
...
Forge 右栏 **Progress** section 只在 ThreadItem 类型为 `todo-list` 时显示
...
| **Phase 2** | Forge 前端：右栏 Sources 区按 source.id 分行展示，可点击 (`rail-projection.ts:76-85` 扩展) | ~30-60 分钟 |
...
- ⏳ **Phase 2 UI**：Forge 右栏 Sources 按 source.id 分行（目前一个 MCP server 只占一行）
