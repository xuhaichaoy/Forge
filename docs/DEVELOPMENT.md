...
- `Progress` shows a completed-task summary plus all plan items; right-rail pinned state controls whether the summary rail is visible; `Side chats` shows active parent side-panel threads; `Background tasks` combines real receiver threads and running unified exec commands; `Outputs` and `Sources` show 6 items by default, then show more/less.
