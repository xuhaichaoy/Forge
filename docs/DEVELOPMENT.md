...
This is the single source of truth for developing Forge. Read it before changing code. Keep it practical, current, and non-duplicative.

## 1. Product Boundary

Forge is a Codex Desktop shell, not a new agent runtime.
...
- [docs/dev/codex-alignment-plan.md](dev/codex-alignment-plan.md): Plan summary card behavior, 10 i18n strings, and right-rail Plan tab behavior.
...
- [docs/dev/codex-alignment-image-generation.md](dev/codex-alignment-image-generation.md): `case 'generated-image': return null` rule, right-rail Artifacts cap, turn-level gallery, side-panel image preview with lightbox fallback.
...
- Run `node scripts/extract-codex-asar.mjs` to extract `/Applications/Codex.app/Contents/Resources/app.asar` to `/private/tmp/codex-asar`. Pass `--force` to refresh after a Codex update and `--pretty` when a prettified mirror is useful. Override paths with `FORGE_CODEX_ASAR` / `FORGE_CODEX_ASAR_OUT` (the legacy `HICODEX_*` env names remain accepted as compatibility aliases).
...
| Desktop area | Forge implementation | Key rule |
| --- | --- | --- |
...
| `thread-user-message-navigation-rail-*.js` | `packages/ui/src/components/thread-user-message-navigation-rail.tsx` + `thread-scroll-layout.tsx` + `conversation-view.tsx` | Desktop shows the floating left-side user-message navigation rail only when a thread has at least four user messages. It renders one 36px × 10px row per user message with a 30px × 2px marker wrapper, portals the rail into a scroll-viewport overlay so `top: 50%` is viewport-relative rather than transcript-height-relative, positions it at `left: 12px`, and opens a right-side hover/focus tooltip anchored to the hovered marker. The tooltip title is the user message label, the body is the first following assistant response after directive-line removal and markdown-to-plain-text projection, and the output row shows at most two turn outputs plus a `+N` remainder count. This is distinct from user `comments` / `commentAttachments` hover cards and from assistant resource cards. |
| `thread-page-header-*.js` + `thread-layout-*.js` + `thread-scroll-layout-*.js` + `scroll-to-bottom-buton-*.js` | `packages/ui/src/components/conversation-chrome.tsx` + `thread-scroll-layout.tsx` + `ForgeApp.tsx` | Conversation chrome is a single compact row: project chip plus optional start/secondary/trailing affordances. Never render a second `cwd` row, an editable cwd input, or a chrome top-right turn-running pill. The workspace path appears at most once across chrome, empty-state hero, and toolbar combined. Turn-running is signaled via the scroll-to-bottom button's working-dots, not as chrome chrome. Always-visible kebab/`MoreHorizontal` thread-action menus do not belong in chrome; thread actions live on the sidebar row context-menu and on turn-row affordances. Conversation content and composer share one thread scroll system. Content and footer use Desktop's current thread tokens (`--thread-content-max-width: none`, toolbar padding 16px, chat font 13px); scroll container uses Desktop's `flex-col-reverse` distance-from-bottom semantics where bottom is `scrollTop = 0`; footer uses `data-thread-scroll-footer`, sticky bottom placement, focus-within scroll-padding reset, measured footer height plus 16px scroll padding, page-scoped distance restoration, 24px bottom-distance threshold, scroll controller callbacks, and a 32px centered scroll-to-bottom button. |
| `loading-page-*.js` + `composer-*.js` start-conversation flow | `packages/ui/src/hooks/use-turn-submission.ts` + `ForgeApp.tsx` | Before a local conversation is materialized, the main transcript area should show a centered loading shell for runtime connection and `thread/start` handoff states. This is a transient state surface only; do not replace it with a marketing or onboarding page. |
| `app-shell-*.js` + `app-main-*.js` + `sidebar-signals-*.js` + `sidebar-project-groups-*.js` + `sidebar-thread-keys-*.js` + `use-workspace-file-search-*.js` | `packages/ui/src/components/sidebar.tsx` + `packages/ui/src/components/command-panel.tsx` + `packages/ui/src/state/sidebar-projection.ts` + `sidebar-preferences.ts` | Left thread rail follows Desktop sizing, top nav order, Search-as-command-menu behavior, account/profile footer before Settings, persisted organize/sort/collapsed/section preferences, `updated_at` sorting, sub-agent filtering, project/recent/current-workspace grouping, compact relative update time, and row status projection. Cmd/Ctrl+K opens the global command menu; native menu events also route Search Chats and Search Files into the matching command-menu modes. Search Chats uses `thread/search` with trimmed `searchTerm`, `archived: false`, `sourceKinds: []`, the sidebar sort key, and content-match snippets merged over loaded pinned/recent chats; Search Files uses backend-backed workspace file search (`fuzzyFileSearch`). File results attach Desktop-style composer mentions instead of inventing a new `UserInput` type. `codex://threads/{id}` URLs route to known threads or resume/read by id once the shell delivers the URL. Header and row actions are hover/focus affordances; active row title double-click opens rename, and row menus include Open in new window when the host flow exists. Omit only actions that still need missing Forge host/protocol flows such as new worktree, and do not add badges or disabled nav items without matching Desktop data. |
...
| `app-initial~app-main~...` `markdown.fileCitation.*` renderer | `packages/ui/src/components/message-file-citations.tsx` + `conversation-markdown-*` | Inline file citations support the legacy `【F:path†Lx-Ly】` marker but should preserve Desktop's structured `artifactCitation.target` semantics when projection provides them. A document target renders page labels, presentation targets render slide/object labels, workbook targets render sheet/range or sheet/object labels, and those labels take precedence over ordinary line labels. Desktop hides `L1` only for non-code-like file kinds; code citations keep `line 1`. Extensionless structured artifact citations expose the Desktop fallback type label in the accessible name, not as visible chip text. |
...
| `markdown-*.js` Mermaid fence path | `packages/ui/src/components/message-unit.tsx::MermaidDiagram` + `sanitizeMermaidCode` | `mermaid` fences render through the public Mermaid package with Desktop-style strict security, deterministic ids, `htmlLabels: false`, stripped `click` bindings, and blocked `securityLevel` overrides. Forge keeps a clean-room flowchart preview/code fallback for failed renders; do not copy bundled Mermaid implementation from `app.asar`. |
...
| `locale-resolver-*.js` + `use-resolved-theme-variant-*.js` + `app-main-*.js` | `packages/ui/src/state/i18n.ts` + `components/i18n-provider.tsx` + `state/theme.ts` + `ForgeApp.tsx` | Locale and theme are app-shell preferences, not per-thread facts. Forge resolves `en-US` / `zh-CN` from stored/browser locale, exposes a local `formatMessage` provider for `hc.*` IDs, persists `system` / `light` / `dark` appearance locally, follows `prefers-color-scheme` in system mode, writes `data-theme` / `data-theme-mode` on the shell root, and `/theme` opens the command panel without requiring app-server connectivity. |
| `app-main-*.js` feedback form | `packages/ui/src/state/slash-request-workflow.ts` + `command-panel.ts` | `/feedback` opens a local feedback report panel. Until Forge owns Codex Desktop's upload endpoint, the supported path is to copy diagnostics (thread id, workspace, runtime status, recent UI logs) and open the Codex GitHub issue template with the thread id in the `steps` parameter. |
| `welcome-page-*.js` + `onboarding-state-*.js` + `nux-gate-*.js` | `packages/ui/src/state/onboarding.ts` + `components/onboarding-empty-state.tsx` | First-run state uses Desktop's durable local keys (`last_completed_onboarding`, `electron:onboarding-projectless-completed`, `electron:onboarding-welcome-pending`, `electron:onboarding-hide-first-new-thread-promos`). Forge keeps this as a lightweight pre-conversation empty state and dismissible first-new-thread banner; do not replace the thread surface with a marketing landing page. The empty-state hero must use Desktop ICU slogans (`home.hero.letsBuild`, `home.hero.whatShouldWeBuild`, `home.hero.whatShouldWeWorkOnInProject`) rather than ad hoc copy such as `Start a chat`, and must not duplicate the composer's `+ New chat` / `Choose folder` affordances while the composer is mounted. The workspace path appears at most once across hero, chrome, and toolbar combined. |
| `prompt-editor-*.js` + `use-file-mention-autocomplete-*.js` + `use-workspace-file-search-*.js` + `use-skills-*.js` + `apps-queries-*.js` + `use-plugins-*.js` | `packages/ui/src/components/composer.tsx` + `prompt-editor.tsx` + `composer-workflow.ts` + `mention-options.ts` + `ForgeApp.tsx` | `@` on the active input line opens the inline mention picker. File candidates come from app-server fuzzy search; skills, apps, and plugins come from their list endpoints and insert Desktop-style prompt links (`$skill`, `$app`, `@plugin`) rather than new protocol-level `UserInput` types. Prompt mention chips must survive ProseMirror serialization/deserialization. |
| Right rail + `composer-*.js` | `packages/ui/src/state/right-rail.ts` + `rail-projection.ts` + `background-agents.ts` + `background-terminals.ts` + `components/right-rail.tsx` + `ForgeApp.tsx` | Section order is Scheduled, Environment, Plan, Outputs, Side chats, Subagents, Tasks, Browser, Sources. Environment and Outputs are mutually exclusive: Environment is shown for Git/environment-backed thread context, while Outputs is the non-environment output/artifact section. Scheduled, Environment, Plan, Side chats, Subagents, Tasks, and Browser collapse when empty; mounted Outputs/Sources render Desktop empty rows such as `No artifacts yet` / `No sources yet`. `/status` is a composer-local panel, not a right-rail footer or section. Current Desktop does not mount a right-rail Progress section; `todo-list` belongs to turn content/fixed in-progress content and the current Plan rail entry, not a Progress rail section. The Plan rail row uses the extracted plan title for its label and opens a right side-panel Plan tab with the current plan markdown; transcript Plan summary cards use the same Plan tab action. Side chats show active parent side-panel threads and use a spinner when in progress; Background tasks are split into Desktop titles `Subagents` and `Tasks`, with terminal stop as a row action. Outputs/Sources use a 6-item preview. Sources render tool entries first, then URL-level web page entries extracted from webSearch `openPage` / `findInPage` http(s) actions; if web searches happened but no safe page URL exists, Sources falls back to one generic `Web search` entry. Projectless generated-image artifact projection must receive the thread cwd plus projectless `outputs/` directory; with generated-image inclusion disabled, only resources under that output directory survive the Desktop resource-path gate. Generated-image Outputs entries preserve explicit generated-image artifact type metadata rather than inferring from `ig_*` filenames; their row labels use Desktop's generated image numbering and their click path opens the generated-image preview tab first, falling back to ordinary artifact/file preview only when no renderable image source exists. The rail is a responsive floating panel with Desktop side-space thresholds: overlay below 180px, shift below 400px, gutter at 400px and above. Pinned state is persisted; when unpinned the summary rail is hidden, and the pin toggle lives in the conversation header rather than inside the rail. Headers are sticky text titles with Desktop after-actions where present, such as Environment's disabled/current `+` selector placeholder and Tasks' process-list action placeholder, and sections are separated by inset rules. Workspace file tree, file mentions, and Outputs/Sources share Desktop's `get-file-icon` extension/MIME key map; glyphs remain monochrome clean-room approximations. File/artifact/Plan side-panel tab ids are host-scoped (`local` for the current local-only shell), workspace file/source tab kind is `workspaceFile:${hostId}`, source tabs expose a toolbar Open file action plus the Desktop workspace-file options subset (copy path, copy contents, rich/raw markdown preview, word wrap; reveal stays in the tab context menu until a toolbar target exists), source tabs use `review.fileSource.*` loading/error/too-large states and preview PDF/markdown through dedicated PDF/rich renderers before falling back to raw text, artifact `View source` converts the existing artifact tab into a pinned source tab in place instead of opening a sibling `file:*` tab, artifact preview tabs expose Open options with an `Open in folder` file-manager action, and open workspace-file tabs are watched through app-server `fs/watch` / `fs/changed` so previews refresh when the source file changes. |
...
- Apps and connector OAuth: app metadata comes from paged `app/list`; connector enablement writes `apps.{id}.enabled`; `app/list/updated`, `mcpServer/oauthLogin/completed`, and claimed app-connect OAuth callbacks invalidate app/MCP surfaces. Desktop connector OAuth completion uses the HTTP callback path `/aip/connectors/links/oauth/callback`; Forge intentionally claims `codex://app-connect-oauth...` and browser callback URLs, de-dupes by OAuth `state`, and refreshes app/plugin panels after the callback. Remaining parity is the product-owned connector start/finish HTTP bridge (`/aip/connectors/links/oauth` and `/callback`) or a protocol-owned equivalent; do not invent local `app/authenticateUrl` / `app/openUrl` JSON-RPC methods from screenshots or old notes.
- Native option/context pickers: Forge supports dynamic `item/tool/call` `request_option_picker`, `request_onboarding_input`, and `setup_codex_step` because the local generated protocol exposes `item/tool/call`, and it supports source-backed dynamic/native setup-context picker responses when app-server provides source rows. `setup_codex_step` task suggestions and context source browsing depend on Desktop's local onboarding/app/plugin state; Forge must keep the protocol response shape but must not fake missing product-owned suggestion/source data. Native `item/tool/requestOptionPicker`, plus full setup-context Browse all, app install, connector OAuth, plugin, and folder picker flows, remain gated on generated protocol and host/product bridges; do not fake setup-context data from screenshots.
- Prior conversation context: Desktop renders `Previous context` / `References prior conversation` when its upstream task input contains a `prior_conversation` item. Current local app-server `UserInput` does not expose that item type, while Forge only receives the projected `referencesPriorConversation` boolean chip. Keep this as a chip/pill; implement richer prior-conversation detail only if the generated local protocol or a host bridge provides the referenced conversation payload. Local fork provenance remains the separate `forkedFromId` / `Parent chat` / `Forked from conversation` path.
- User comment / annotation attachment popovers: Desktop's large sent-user hover card is the `comments` / `commentAttachments` popover for browser, PDF, artifact, diff, and design-tweak annotations. It lives in the user attachment strip above the bubble, not in the lower message meta/action row; the popover opens on hover/focus with Desktop's top/start placement and short close delay. It needs per-attachment body text, origin, path/line/range metadata, optional preview thumbnails, and `cwd`/`hostId`; `commentCount` alone is not enough. Forge renders this hover card only when a user-message item actually carries `comments` / `commentAttachments` details, and file rows inside reuse the same `data-file-reference` / `openInSidePanel` workspace-file path as inline file citations. Do not fabricate this popover from `commentCount`, prior-conversation text, or summary chips.
...
- High-fidelity Office previews: Forge may offer lower-fidelity previews or OS/native open behavior, but do not chase Codex Desktop's high-fidelity docx/pptx renderer in Tauri unless product explicitly accepts the runtime cost and maintenance surface.
...
- Protocol `sleep` ThreadItems stay out of transcript projection.
- Right rail order is `Scheduled -> Environment -> Plan -> Outputs -> Side chats -> Subagents -> Tasks -> Browser -> Sources`; Plan rows open the right side-panel Plan tab, and `/status` opens the composer-local Status panel.
- Environment and Outputs are mutually exclusive; do not render both summary sections in one rail.
- Empty Scheduled, Environment, Side chats, Subagents, Tasks, and Browser sections should be hidden; mounted `Outputs` and `Sources` show Desktop empty-state rows.
- Current Desktop does not mount a right-rail Progress section; local-turn `todo-list` items are hoisted into fixed in-progress content instead of being rendered as a normal transcript row.
- Right-rail pinned state controls whether the summary rail is visible; `Side chats` shows active parent side-panel threads; background work is split into `Subagents` and `Tasks`; `Outputs` and `Sources` show 6 items by default, then show more/less. The side-panel New tab page reuses current output artifacts as Desktop-style `Suggested` rows beneath its action grid.
...
- React components: keep large reusable views out of `ForgeApp.tsx` once they become independently testable.
...
Do not split just to create folders. Do not keep adding code to `ForgeApp.tsx` when a workflow can be named and tested separately.
...
- Default model catalog entry in `crates/host` for isolated Forge home.
- Static UI labels.
...
The effective Codex profile is selected/written by Forge. Codex core should remain upgradeable.
...
- `item/tool/call` including dynamic client-input helpers such as `request_option_picker`, `request_onboarding_input`, `setup_codex_step`, and `setup_codex_context_picker`
...
- Dynamic `request_onboarding_input` requests return the same wrapper with JSON `{ answers }`; Dismiss/Stop return `{ answers: {} }` instead of interrupting the turn.
- Dynamic `setup_codex_step` requests return the same wrapper. Role step returns JSON `{ action, selectedRoles }` with Desktop onboarding role ids; task step returns `{ action, answers }` with `first_task`; context step returns `{ action, selectedSources }`; `complete` is not a pending request.
- Dynamic `setup_codex_context_picker` requests return the same tool-call wrapper with JSON `{ action, selectedSources }`; Continue returns sanitized selected source ids plus connected current-row sources, while Skip/Dismiss return an empty array.
...
Forge is an operational desktop tool. It should feel quiet, dense, and repeatable.
...
Forge-only settings surfaces that have no Desktop counterpart (Models, Images, Approvals, Experimental, notifications) are legitimate local additions. Document each in §13 rather than adding ad hoc panels — every Forge-only surface must be reachable from a single Settings entry and must not impersonate a Desktop section name.
...
When `tauri:dev` fails on port `5178`, first check whether the existing server is already Forge. The dev script may reuse it.
...
- Read this guide.
- Identify the source of truth: app-server protocol, Codex Desktop chunk, or existing Forge code.
...
- `data-thread-find-target`, `data-thread-find-composer`, `data-thread-find-skip` — Forge-only plumbing for the local find-in-thread bar (match COUNTS come from state; these attributes only scope the DOM marking/scroll pass). Desktop's equivalent lives in `review-file-tree-side-pane-*.js` with a different attribute scheme; do not chase Desktop names.
- `data-right-rail-mode` — Forge right-rail responsive mode (`overlay` / `shift` / `gutter`). Driven by `RIGHT_RAIL_*` breakpoints in `state/right-rail.ts`. Desktop does not encode the mode as a DOM attribute.
- `data-csp-*` (six attributes on MCP widget frames). **Security contract** — these define the per-frame CSP envelope Forge applies before mounting an MCP app widget. They must stay in sync with the iframe sandbox flags. Changing them is a security review surface, not a styling change.

Hooks and components:

- `useDismissibleLayer` — a 28-line minimal click-outside / Esc hook used while Forge has no Floating-UI installation. When a real popover primitive lands (see right-rail and chip popover work), migrate consumers and delete this hook.
...
Forge-only settings surfaces (referenced from §9):
...
- The sidecar lifecycle pill is Forge-only and lives in the sidebar footer / transient toasts, never in chrome top-right.
- Running turn plan/diff previews render as fixed in-progress conversation content between the transcript and composer, not through the above-composer portal or right rail. The diff preview is a compact inline changed-files chip; do not inject a raw diff preview into transcript render groups.
- The `+ New chat` / `Choose folder` empty-state buttons must only appear when there is no mounted composer; never alongside one.

When adding a new Forge-only primitive, add it to this section in the same PR. If the addition replaces a Desktop primitive, prefer renaming Forge usage to match the Desktop name rather than maintaining a parallel Forge-only abstraction.
