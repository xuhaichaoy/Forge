...
- Right rail order is `Progress -> Automations -> Environment -> Outputs -> Side chats -> Background tasks -> Browser -> Sources -> Status`.
- Environment and Outputs are mutually exclusive; do not render both summary sections in one rail.
- Empty `Progress`, Automations, Environment, Side chats, Background tasks, Browser, and Status sections should be hidden; mounted `Outputs` and `Sources` show Desktop empty-state rows.
