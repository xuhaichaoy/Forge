...
| Right rail + `composer-*.js` | `packages/ui/src/state/right-rail.ts` + `rail-projection.ts` + `background-agents.ts` + `background-terminals.ts` + `components/right-rail.tsx` + `ForgeApp.tsx` | Section order is Automations, Environment, Outputs, Side chats, Subagents, Tasks, Browser, Sources. Environment and Outputs are mutually exclusive: Environment is shown for Git/environment-backed thread context, while Outputs is the non-environment output/artifact section. Automations, Environment, Side chats, Subagents, Tasks, and Browser collapse when empty; mounted Outputs/Sources render Desktop empty rows such as `No artifacts yet` / `No sources yet`. `/status` is a composer-local panel, not a right-rail footer or section. Current Desktop does not mount a right-rail Progress section; `todo-list` belongs to turn content/fixed in-progress content, not the rail. Side chats show active parent side-panel threads and use a spinner when in progress; Background tasks are split into Desktop titles `Subagents` and `Tasks`, with terminal stop as a row action. Outputs/Sources use a 6-item preview. The rail is a responsive floating panel with Desktop side-space thresholds: overlay below 180px, shift below 400px, gutter at 400px and above. Pinned state is persisted; when unpinned the summary rail is hidden, and the pin toggle lives in the conversation header rather than inside the rail. Headers are sticky, text-only, and separated by inset rules. Workspace file tree, file mentions, and Outputs/Sources share Desktop's `get-file-icon` extension/MIME key map; glyphs remain monochrome clean-room approximations. File/artifact side-panel tab ids are host-scoped (`local` for the current local-only shell), workspace file/source tab kind is `workspaceFile:${hostId}`, source tab context menus use the Desktop workspace-file actions subset (`Open file`, copy path, copy contents, reveal), artifact `View source` converts the existing artifact tab into a pinned source tab in place instead of opening a sibling `file:*` tab, artifact preview tabs expose Open options with an `Open in folder` file-manager action, and open workspace-file tabs are watched through app-server `fs/watch` / `fs/changed` so previews refresh when the source file changes. |
...
- Right rail order is `Automations -> Environment -> Outputs -> Side chats -> Subagents -> Tasks -> Browser -> Sources`; `/status` opens the composer-local Status panel.
...
- Right-rail pinned state controls whether the summary rail is visible; `Side chats` shows active parent side-panel threads; background work is split into `Subagents` and `Tasks`; `Outputs` and `Sources` show 6 items by default, then show more/less.
