...
| `app-server-manager-signals-*.js` + `local-conversation-thread-*.js` | `packages/ui/src/state/thread-workflow.ts::forkThreadFromTurn` + `message-unit.tsx` | Assistant fork action uses the Desktop sequence: `thread/fork` from latest, then `thread/rollback` the forked thread by the number of later turns. Turn rating / feedback thumbs are product-disabled in Forge; do not render or reintroduce Good response / Bad response controls or the feedback modal. |
...
| Right rail + `composer-*.js` | `packages/ui/src/state/right-rail.ts` + `rail-projection.ts` + `background-agents.ts` + `background-terminals.ts` + `components/right-rail.tsx` + `ForgeApp.tsx` | Section order is Automations, Environment, Plan, Outputs, Side chats, Subagents, Tasks, Browser, Sources. Environment and Outputs are mutually exclusive: Environment is shown for Git/environment-backed thread context, while Outputs is the non-environment output/artifact section. Automations, Environment, Plan, Side chats, Subagents, Tasks, and Browser collapse when empty; mounted Outputs/Sources render Desktop empty rows such as `No artifacts yet` / `No sources yet`. `/status` is a composer-local panel, not a right-rail footer or section. Current Desktop does not mount a right-rail Progress section; `todo-list` belongs to turn content/fixed in-progress content and the current Plan rail entry, not a Progress rail section. The Plan rail row uses the extracted plan title for its label and opens a right side-panel Plan tab with the current plan markdown. Side chats show active parent side-panel threads and use a spinner when in progress; Background tasks are split into Desktop titles `Subagents` and `Tasks`, with terminal stop as a row action. Outputs/Sources use a 6-item preview. Sources render tool entries first, then URL-level web page entries extracted from webSearch `openPage` / `findInPage` http(s) actions; if web searches happened but no safe page URL exists, Sources falls back to one generic `Web search` entry. Projectless generated-image artifact projection must receive the thread cwd plus projectless `outputs/` directory; with generated-image inclusion disabled, only resources under that output directory survive the Desktop resource-path gate. The rail is a responsive floating panel with Desktop side-space thresholds: overlay below 180px, shift below 400px, gutter at 400px and above. Pinned state is persisted; when unpinned the summary rail is hidden, and the pin toggle lives in the conversation header rather than inside the rail. Headers are sticky text titles with Desktop after-actions where present, such as Environment's disabled/current `+` selector placeholder and Tasks' process-list action placeholder, and sections are separated by inset rules. Workspace file tree, file mentions, and Outputs/Sources share Desktop's `get-file-icon` extension/MIME key map; glyphs remain monochrome clean-room approximations. File/artifact/Plan side-panel tab ids are host-scoped (`local` for the current local-only shell), workspace file/source tab kind is `workspaceFile:${hostId}`, source tab context menus use the Desktop workspace-file actions subset (`Open file`, copy path, copy contents, reveal), artifact `View source` converts the existing artifact tab into a pinned source tab in place instead of opening a sibling `file:*` tab, artifact preview tabs expose Open options with an `Open in folder` file-manager action, and open workspace-file tabs are watched through app-server `fs/watch` / `fs/changed` so previews refresh when the source file changes. |
...
- Right rail order is `Automations -> Environment -> Plan -> Outputs -> Side chats -> Subagents -> Tasks -> Browser -> Sources`; Plan rows open the right side-panel Plan tab, and `/status` opens the composer-local Status panel.
- Environment and Outputs are mutually exclusive; do not render both summary sections in one rail.
- Empty Automations, Environment, Side chats, Subagents, Tasks, and Browser sections should be hidden; mounted `Outputs` and `Sources` show Desktop empty-state rows.
